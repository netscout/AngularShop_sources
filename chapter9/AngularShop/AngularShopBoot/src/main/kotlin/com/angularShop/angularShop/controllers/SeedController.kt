...
class SeedController(
        ...
        @Autowired private var roleService: RoleService,
        @Autowired private var userRoleService: UserRoleService
) {
    @Transactional
    @GetMapping("/GenerateOrders")
    fun generateOrders() : ResponseEntity<MutableMap<String, Any>> {
        var appliedCount = 0
        for (os in OrderStatuses.values()) {
        ...
        model["appliedCount"] = appliedCount
    ...
    fun generateUsers() : ResponseEntity<MutableMap<String, Any>> {
        var admin = User(
        ...
        user = registerUser(user, false)

        val model: MutableMap<String, Any> = HashMap()
        model["count"] = 2
        model["users"] = listOf(admin, user)

        return ResponseEntity.ok(model)
    }

    private fun registerUser(user: User, isAdmin: Boolean = true)
            : User {
        ...
        val roles = if(isAdmin)
            ...
            mutableListOf(roleService.getRoleByName("USER"))

        for(role in roles) {
            userRoleService.addUserRole(
                    UserRole(user.id!!, role.id!!))
        }
