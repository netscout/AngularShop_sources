...
from shop.models import Category, Maker, \
    Order, OrderItem, OrderStatusId, \
        Product, ProductCategory, ProductImage, User
...
class OrderItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model=OrderItem
        fields = ['id', 'qty', 'price', 'product_name']

class OrderSerializer(serializers.ModelSerializer):
    #order_status_name = serializers.SerializerMethodField()
    order_status_id = serializers.IntegerField(
        source='order_status'
    )
    #외래키로 참조하는 사용자의 이름만 읽기 전용으로 가져오기
    ...
    def get_order_status_name(self, obj):
        return OrderStatusId(obj.order_status).label

    class Meta:
        model = Order
        fields = [
            'id', 'to_name', 'address1', 'address2', 
            'phone', 'total_price', 'created_date', 
            'modified_date', 'order_status', 'user_name',
            'user_id', 'order_status_id', 'order_items']
